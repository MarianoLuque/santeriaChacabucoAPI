...
const getItem = async (req, res) => {
    try {
        req = matchedData(req)
        const {id} = req
        const data = await coloresModel.findById({_id: id})
        res.send({data})
    } catch (err) {
...
const updateItems = async (req, res) => {
    try {
        ...
        const data = await coloresModel.findOneAndUpdate({_id: id}, body, {new: true})
        res.send({data})
    } catch (err) {
...
const deleteItems = async (req, res) => {
    try {
        req = matchedData(req)
        const {id} = req
        const data = await coloresModel.findByIdAndDelete({_id: id})
        res.send({data})
    } catch (err) {
